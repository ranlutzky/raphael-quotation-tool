...
export default function QuotationApp() {
  const loadSavedData = () => {
    try {
      const saved = localStorage.getItem("RAPHAEL_APP_V4"); // Changed key
  ...
  const loadSavedCustomers = () => {
    try {
      const saved = localStorage.getItem("RAPHAEL_CUSTOMERS_V3");
  ...
  useEffect(() => {
    ...
    localStorage.setItem("RAPHAEL_APP_V4", JSON.stringify(dataToSave)); // New Key
  ...
  const saveCustomerToList = (customerName) => {
    ...
    if (trimmedName && !customerList.includes(trimmedName)) {
      ...
      localStorage.setItem("RAPHAEL_CUSTOMERS_V3", JSON.stringify(newList));
    }
  };
